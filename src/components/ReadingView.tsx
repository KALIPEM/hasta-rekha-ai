...
export function ReadingView({ reading, onBack, onStart, onUpdateTitle }: Props) {
  ...
  return <article className={isCouple?'report-page section-width couple-report-page':'report-page section-width'}>
    ...
    <header className="report-heading"><div className="eyebrow">{reading.mode === 'roast' ? <Flame size={15}/> : isCouple ? <Heart size={15}/> : <Sparkles size={15}/>} {reading.mode === 'roast' ? 'YOUR COSMIC COMEDY' : isCouple ? 'YOUR COUPLE READING' : 'YOUR PALM READING'}</div>
    ...
    {content ? <>
      <section className={isCouple?'report-summary couple-report-summary':'report-summary'}><div><span className="eyebrow">{isCouple?'THE THREAD BETWEEN YOU':'THE THREAD THROUGH YOUR STORY'}</span><h2>{content.majorHighlight}</h2><p className="report-hook">{content.openingHook}</p><p>{content.executiveSummary}</p></div>{isCouple?<div className="couple-report-emblem" aria-hidden="true"><span>✦</span><Heart size={42}/><span>✦</span></div>:<PalmIllustration small/>}</section>
      <nav className="report-jump-links" aria-label="Reading sections"><a href="#your-perspectives">{isCouple?'Your connection':'Your perspectives'}</a>{!!content.lifeAreas?.length && <a href="#life-questions">Love, money & life</a>}{content.lifeTimeline.length > 0 && <a href="#reflection-timeline">Past, present & future</a>}<a href="#small-steps">{isCouple?'Shared next steps':'Small steps'}</a></nav>
      {!!content.lifeAreas?.length && <section id="life-questions" className="report-section life-questions"><div className="eyebrow">WHAT YOUR PALM REVEALS</div><h2>Love, money & the life around you.</h2><p className="muted">The patterns shaping your path, and what to do with them.</p>{content.lifeAreas.map(area=><details className="life-area" key={area.id}><summary><span>{area.title}</span><span className="life-area-toggle" aria-hidden="true"/></summary><p className="aspect-summary">{area.summary}</p>{area.insights.map((insight,index)=><p className="life-insight" key={index}>{insight}</p>)}<div className="life-takeaways"><div><h4>Watch out for</h4><p>{area.watchOutFor}</p></div><div><h4>Try this</h4><p>{area.nextStep}</p></div></div></details>)}</section>}
      <section id="your-perspectives" className="report-section"><div className="eyebrow">{isCouple?'THE WAY YOUR PALMS MEET':'THE LINES THAT STAND OUT'}</div><h2>{isCouple?'Your shared dynamics.':'Your reading.'}</h2><div className={isCouple?'aspect-grid couple-aspect-grid':'aspect-grid'}>{content.aspects.map((a, i) => <section className="aspect-card" key={i}><span className="aspect-number">0{i+1}</span><h3>{a.aspectName}</h3><p className="aspect-summary">{a.summary}</p><p>{a.detailedInterpretation}</p><div className="palm-evidence"><Eye size={16}/><div><span>{isCouple?'PALMS CONSIDERED':'PALM DETAIL'}</span><p>{a.palmEvidence}</p></div></div></section>)}</div></section>
