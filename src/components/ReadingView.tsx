...
export function ReadingView({ reading, onBack, onStart, onUpdateTitle }: Props) {
  ...
  async function share(line:string,copy=false) {
    try {
      const text=shareCaption(line,reading.mode==='roast');
  ...
  function download() {
    const text = content ? [
      ...
      '## Reflection timeline', ...content.lifeTimeline.flatMap(t => [t.ageRange + ': ' + t.phaseName, t.keyEventOrShift, t.palmEvidence, '']),
      ...
      'For entertainment and self-reflection. Palmistry is not a scientific assessment or a prediction.'
    ].join('\n') : reading.readingText;
  ...
  return <article className="report-page section-width">
    ...
    <header className="report-heading"><div className="eyebrow">{reading.mode === 'roast' ? <Flame size={15}/> : <Sparkles size={15}/>} {reading.mode === 'roast' ? 'YOUR COSMIC COMEDY' : reading.mainFocus === 'Couple compatibility' ? 'YOUR COUPLE REFLECTION' : 'YOUR VEDIC REFLECTION'}</div>
    ...
    {content ? <>
      ...
      {content.lifeTimeline.length > 0 && <section id="reflection-timeline" className="report-section"><div className="eyebrow">A PLACE TO PAUSE</div><h2>Your reflection timeline.</h2><p className="muted">Prompts to consider your journey, rather than predictions of events.</p><div className="reflection-timeline">{content.lifeTimeline.map((t, i) => <div key={i}><span className="timeline-dot"/><span className="eyebrow">{t.ageRange}</span><h3>{t.phaseName}</h3><p>{t.keyEventOrShift}</p><small>{t.palmEvidence}</small></div>)}</div></section>}
      ...
      <details className="image-notes"><summary>About this photo & interpretation</summary><p><strong>{content.imageQualityCheck.clarity}</strong> · {content.imageQualityCheck.notes}</p><p>{content.imageQualityCheck.confidenceImpact}</p><p>AI can misread visible features. Palmistry associations are traditional interpretations and do not establish facts about your personality, health, or future.</p></details>
    </> : <section className="legacy-reading markdown-body"><ReactMarkdown>{reading.readingText}</ReactMarkdown></section>}
    <section id="share-lines" className="report-section"><div className="eyebrow">A LITTLE SOMETHING FOR THE GROUP CHAT</div><h2>{reading.mode==='roast'?'Take the joke with you.':'A thought worth sharing.'}</h2><p className="muted">Pick a caption that feels like you. These ready-made lines match broad reading themes; they don’t include your photos, report, names or private details.</p><div className="share-lines-grid">{shareLines.map(line=><section className="share-line-card" key={line.theme}><span className="eyebrow">{line.theme}</span><blockquote>{line.text}</blockquote><small>{reading.mode==='roast'?'Playful roast · not a personal fact':'Reflection · not a prediction'}</small><div><button className="button button-brand button-small" onClick={()=>void share(line.text)}><Share2 size={15}/> Share this line</button><button className="button button-outline button-small" onClick={()=>void share(line.text,true)}>Copy caption</button></div></section>)}</div><p className="muted">Sharing opens your device’s share options, or copies the caption if sharing isn’t supported. You choose where to post.</p></section>
