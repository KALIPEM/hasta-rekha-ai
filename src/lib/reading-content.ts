...
export function parseReadingContent(value: unknown): ReadingContent {
  ...
  if (!data || !text(data.openingHook) || !text(data.majorHighlight) || !text(data.executiveSummary)
    ...
    || (data.lifeAreas !== undefined && (!list(data.lifeAreas,a=>a && ['id','title','summary','watchOutFor','nextStep','basis'].every(k=>text(a[k])) && list(a.questions,q=>q && text(q.question) && text(q.answer)) && a.questions.length>0) || new Set(data.lifeAreas.map((a:any)=>a.id)).size!==data.lifeAreas.length))) {
