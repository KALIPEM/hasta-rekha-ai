...
test('Azure image request uses strict structured output, server credentials and bounded output',async()=>{
  ...
  const fetcher: typeof fetch = async(url,init)=>{
    calls++;
    if(calls===3)return reply(Object.fromEntries(LIFE_TOPICS.map(t=>[t.id,{summary:'A useful reflection.',answerOne:'Consider what matters to you.',answerTwo:'Discuss your options.',watchOutFor:'If you are rushing, pause.',nextStep:'Write down one priority.'}])));
