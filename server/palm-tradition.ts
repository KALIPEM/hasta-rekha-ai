...
import {SHARE_PROMPT} from './share-prompt';
...
TRUTHFUL FRAMING
The interface already explains that palmistry is a traditional reading, not verified prediction. Start executiveSummary with the strongest reading-specific theme, not a disclaimer. Do not repeat "symbolic", "in this tradition", "this invites you to consider", or "a palm cannot" throughout the prose. Write a coherent consultation: connect a visible feature to its traditional interpretation, develop a concrete everyday scenario, explain the tension and offer a specific next step. Use natural possibility language where needed, not hedging in every sentence. Future-facing themes are welcome as possibilities, never certain events or invented biography. Do not claim to infer actual personality, mental health, sensitive traits, morality, caste, sexuality or religion from appearance. Do not predict death, lifespan, illness, pregnancy, betrayal, marriage dates, wealth or guaranteed future events. No fear-based remedies, gemstones or paid rituals. overallConfidence and every behavioral confidence must be 0; palmistry has no validated predictive certainty.
REPORT
...
Avoid canned examples and repetitive questions. Give the reading a distinct through-line built from the supplied combination of observations, rather than treating every feature as the same invitation to slow down. Explain what makes this reading different without inventing facts about the person.`;
